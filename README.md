...
scripts/
  ...
  render-image.js      # 리포트 → 게시용 이미지 카드(PNG) 렌더링
  post-aptner.js       # Playwright로 아파트너 로그인 후 게시 (이미지 카드 자동 첨부)
data/
...
- 매주 **월·목 08:30 KST** 자동 실행: 데이터 갱신 → 웹페이지 리포트 커밋 → 새 소식(수원 신고가 또는 우리 아파트 새 거래)이 있으면 이미지 카드를 만들어 아파트너 게시(텍스트 + 이미지 첨부)
